...
jQuery(function ($) {
  var status = $("#status").css({
      backgroundColor: "#121E34",
      color: "#fff",
    }),
    button = $(".btn-primary"),
    form_container = $(".form-container").css({
      backgroundColor: "#121E34",
      height: "100%",
    }),
    waiter = $("#waiter").css({
    ...
    term_type = $("#term").css("background-color", "#121E34"),
    style = {},
  ...
  function ajax_complete_callback(resp) {
    ...
    var ws_url = window.location.href.split(/\?|#/, 1)[0].replace("http", "ws"),
      ...
      termOptions = {
        ...
        theme: {
          background: "#121E34",
          foreground: "#FFFFFF", //cores de fundo
        },
      };
  ...
  parse_url_data(
    decode_uri_component(window.location.search.substring(1)) +
      "&" +
      decode_uri_component(window.location.hash.substring(1)),
    form_keys,
    opts_keys,
    url_form_data,
    url_opts_data
